...
ENCHANT = path.join(get_python_lib(), "enchant")
if not path.exists(ENCHANT):
	raise RuntimeError("Please use pip to install sphinx, pyenchant, and pygtkspellcheck.")
...
files = glob.iglob(path.join(ENCHANT, "*.dll"))
for file in files:
	shutil.copy(file, EXE_ROOT)

distutils.dir_util.copy_tree(path.join(ENCHANT, "share", "enchant"), path.join(EXE_ROOT, "share", "enchant"), update=1)
distutils.dir_util.copy_tree(path.join(ENCHANT, "lib", "enchant"), path.join(EXE_ROOT, "lib", "enchant"), update=1)
